<script lang="ts" setup>
  ...
  const rules = computed<FormRules>(() => {
    const isPayEnabled = formInline.payEpayStatus === '1';

    return {
      payEpayStatus: [{ required: true, trigger: 'change', message: '请选择当前支付开启状态' }],
      payEpaySecret: [{ required: isPayEnabled, trigger: 'blur', message: '请填写支付秘钥' }],
      payEpayPid: [{ required: isPayEnabled, trigger: 'blur', message: '请填写商户PID' }],
      payEpayNotifyUrl: [
      ...
      payEpayApiPayUrl: [
        { required: isPayEnabled, trigger: 'blur', message: '请填写平台支付API请求地址' },
      ],
      payEpayApiQueryUrl: [
        { required: isPayEnabled, trigger: 'blur', message: '请填写平台API商户查询地址' },
      ],
      payEpayRedirect: [
        { required: isPayEnabled, trigger: 'change', message: '请选择是否开启跳转支付' },
      ],
      payEpayChannel: [
  ...
  async function queryAllconfig() {
    ...
    const payEpayChannel = res.data.payEpayChannel ? JSON.parse(res.data.payEpayChannel) : [];
        ...
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="商户PID" prop="payEpayPid">
              <el-input v-model="formInline.payEpayPid" placeholder="请填写商户PID" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="商户秘钥" prop="payEpaySecret">
              <el-input v-model="formInline.payEpaySecret" placeholder="请填写商户秘钥" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :xs="24" :md="24" :lg="24" :xl="24">
            <el-form-item label="开启支付渠道" prop="payEpayChannel">
              <el-checkbox-group v-model="formInline.payEpayChannel" size="small">
                <el-checkbox
