<script lang="ts" setup>
  ...
  const rules = computed<FormRules>(() => {
    const isPayEnabled = formInline.payMpayStatus === '1';

    return {
      payMpayStatus: [{ required: true, trigger: 'change', message: '请选择当前支付开启状态' }],
      payMpaySecret: [{ required: isPayEnabled, trigger: 'blur', message: '请填写支付秘钥' }],
      payMpayPid: [{ required: isPayEnabled, trigger: 'blur', message: '请填写商户PID' }],
      payMpayNotifyUrl: [
  ...
  async function queryAllconfig() {
    ...
    const payMpayChannel = res.data.payMpayChannel ? JSON.parse(res.data.payMpayChannel) : [];
        ...
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="商户PID" prop="payMpayPid">
              <el-input v-model="formInline.payMpayPid" placeholder="请填写商户PID" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="商户秘钥" prop="payMpaySecret">
              <el-input v-model="formInline.payMpaySecret" placeholder="请填写商户秘钥" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :xs="24" :md="24" :lg="24" :xl="24">
            <el-form-item label="开启支付渠道" prop="payMpayChannel">
              <el-checkbox-group v-model="formInline.payMpayChannel" size="small">
                <el-checkbox
