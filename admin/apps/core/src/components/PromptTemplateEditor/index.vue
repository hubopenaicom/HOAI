<script setup lang="ts">
  ...
  // 更新选项值
  const updateOptionValue = (fieldId: string | number, optionIndex: number, value: string) => {
    fields.value = fields.value.map((field) => {
      if (field.id === fieldId && field.type === 'select' && field.options) {
        const options = [...field.options];
  ...
  // 更新 Placeholder 值
  const updatePlaceholderValue = (fieldId: string | number, value: string) => {
    fields.value = fields.value.map((field) => {
      if (field.id === fieldId) {
  ...
  // 更新 Title 值
  const updateTitleValue = (fieldId: string | number, value: string) => {
    fields.value = fields.value.map((field) => {
      if (field.id === fieldId) {
                  ...
                  <el-form-item label="下拉选项">
                    <div class="space-y-2 w-full">
                      ...
                      >
                        <el-input
                          :model-value="option"
                          @update:modelValue="(val) => updateOptionValue(field.id, Number(index), val)"
                          placeholder="选项内容"
                          size="small"
                          clearable
                          class="flex-grow"
                        ...
                        <el-button
                          ...
                          @click="removeOption(field.id, Number(index))"
                        />
                      </div>
                      <el-button
                        :icon="Plus"
